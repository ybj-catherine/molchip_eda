...
│ P1 任务定义                                                              │
├─────────────────────────────────────────────────────────────────────────┤
│ Input:  29 个开源 RTL IP + 1500 条随机 synthesis recipe + Nangate45     │
│         工艺映射配置                                                      │
│ Output: 约 87 万个中间/最终 AIG 图样本、每个样本的图结构与节点/边特征、     │
│         最终节点数/面积/时延标签，以及 QoR 回归/IP 分类预测                │
│ Supervision: 由 Yosys/ABC/OpenROAD 工具链自动产出 AIG 和 PPA 标签；训练 │
│              为监督回归/分类，无需人工逐样本标注                          │
│ Why-hard: 同一 recipe 在不同 IP 上效果差异大；跨 recipe/IP/组合泛化困难；│
│           数据生成约 25 万 compute-hours；中间 AIG 与最终标签需精确对齐； │
│           per-design 标准化可能泄漏测试分布；PyG 版本兼容与路径契约复杂   │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 组件 | 一句话总结 |
|---|---|
| 29 IP dataset | 覆盖通信、控制、密码、DSP、处理器的 29 个开源 RTL IP，规模从 462 到 245k 节点。 |
| 1500 random recipes | 从 7 种 ABC 变换中随机采样、固定长度 20 的合成脚本，用于探索综合序列空间。 |
| AIG trajectory | 每条 recipe 保存 20 个中间/最终 AIG，形成 87 万个带标签的图样本。 |
| GraphML/PyG converter | 将 BENCH 格式 AIG 转为 NetworkX GraphML 再序列化为 PyG Data 对象。 |
| QoR baseline (Net1/2/3) | GCN 编码 AIG + Conv1d 编码 recipe + FC 预测 normalized area/delay/nodes。 |
| IP classifier | 两层 GCN 根据 AIG 结构识别 IP 类别，验证图表示学习能力。 |

---

## 讨论问题

1. baseline 保存了 edge type 却没有在消息传递中使用，这对模型表达能力损失有多大？应如何设计消融验证？
2. Variant 2 的 per-design normalization 需要测试 IP 全部 1500 个真实 QoR，是否与“减少昂贵综合”任务动机冲突？更合理的无泄漏归一化是什么？
3. Net1/2/3 主要预测 normalized MSE，但实际综合更关心 top-k recipe 选择和 Pareto 前沿；应如何设计更贴近应用的评估指标？
