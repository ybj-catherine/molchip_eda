...
> 📚 **177 篇论文深度讲解** · 169 个论文目录 · 主索引 + 增量清单 + 点击直达
> 🗓️ 最近整理：2026-09-06
> 📖 导航：[00-总索引.md](00-总索引.md) · [论文增量整理清单_20260906.md](论文增量整理清单_20260906.md) · 校验：`python3 _校验.py`（177 个文件）

---

## 30 秒速览

本仓库为 AI4EDA（AI for Electronic Design Automation）领域 **177 篇论文** 提供深度技术解读。其中主分类表覆盖 120 篇，其余 57 篇通过增量清单统一导航。
...
当前机器校验结果为 **151 份完全通过、26 份待完善**；待完善项主要是早期增量讲解的章节完整度和输入/输出例子，不代表论文真实性存疑。
...
> 这里保留原有主表口径；60 个增量目录（含已并入主表的 HLS-RTL、HLS-LeVeri、CoEvoP&R）统一见 [论文增量整理清单_20260906.md](论文增量整理清单_20260906.md)。因此主表与增量表有 3 篇交叉，去重后总数为 177。
...
├── 论文增量整理清单_20260906.md       ← 60 个增量论文目录及校验状态
├── 芯片流程.md                       ← 17 阶段标准编号定义（①-⑰）
├── _合并规范.md                      ← 12 节骨架写作规范
├── _校验.py                          ← 机器校验脚本
...
> 📐 校验：`python3 _校验.py`（177 个文件；151 个通过，26 个待完善）
